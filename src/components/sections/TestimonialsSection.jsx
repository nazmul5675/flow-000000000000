...
const NameCard = ({ name, role }) => (
   <div className="bg-[#f6f6f6] p-8 lg:p-10 rounded-[2rem] border-transparent hover:bg-black transition-all duration-500 ease-in-out group/card cursor-pointer">
      <h4 className="text-lg font-sans font-medium text-foreground tracking-tight transition-colors duration-500 group-hover/card:text-white">{name}</h4>
      <p className="text-[9px] font-bold font-sans uppercase tracking-[0.15em] text-foreground/40 mt-1 transition-colors duration-500 group-hover/card:text-white/50">{role}</p>
   </div>
);

const QuoteCard = ({ quote }) => (
   <div className="bg-[#f6f6f6] p-8 lg:p-10 rounded-[2rem] border-transparent flex flex-col min-h-[300px] justify-between transition-all duration-500 ease-in-out hover:bg-black group/quote cursor-pointer">
      <div>
         <div className="flex gap-1 mb-8">
            {[...Array(5)].map((_, i) => (
               <svg key={i} className="w-3 h-3 fill-orange-500 transition-colors duration-500 group-hover/quote:fill-orange-400" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
            ))}
         </div>
         <p className="text-xl lg:text-3xl font-sans font-medium text-foreground/80 leading-snug tracking-tight italic transition-colors duration-500 group-hover/quote:text-white">
            “{quote}”
         </p>
      </div>
      <div className="mt-8 pt-8 border-t border-black/5 transition-colors duration-500 group-hover/quote:border-white/10">
         <p className="text-[9px] font-bold font-sans uppercase tracking-[0.2em] text-foreground/30 transition-colors duration-500 group-hover/quote:text-white/40">“ Great Design Solutions ”</p>
      </div>
   </div>
);
